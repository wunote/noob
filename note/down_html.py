...
def getinfo(addr):
    ...
    url = 'http://'+addr+'/'#通过用户输入的网址连接上网络协议，得到URL。
    print 'Getting>>>>> '+url#打印提示信息，表示正在抓取网站
    websitefilepath = os.path.abspath('.')+'/'+addr#通过函数os.path.abspath得到当前程序所在的绝对路径，然后搭配用户所输入的网址得到用于存储下载网页的文件夹
...
if __name__=="__main__":
    addr = raw_input("Input the Website Address(without \"http:\")>")
    getinfo(addr)
